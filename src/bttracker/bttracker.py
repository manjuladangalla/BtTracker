...
class BtTracker(object):
    ...
    def run(self):
        ...
        if self.adapter is not None:
            ...
            raise ValueError("BtTracker.run can only be called in a context "
                             "(e.g. `with BtTracker(...) as s: s.run()`)")
    ...
    def _cb_properties_changed(self, sender, obj, iface, signal, params):
        ...
        if DEVICE_INTERFACE in params:
            ...
            if device is not None:
                ...
                if device.rssis[-1] < self.threshold_rssi:
                    if device.active == True:
                        device.active = False
                        print_device(device,"Out")
    ...
    def __init__(self, output_path=None, backup_interval=5, resume=False,
                 attempt_connection=False, threshold_rssi=-100,
                 queueing_interval=5):
        ...
        self.adapter = None
        self._log = logging.getLogger("bttarcker.BtTracker")
